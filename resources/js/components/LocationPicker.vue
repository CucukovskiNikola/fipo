  ...
  try {
    ...
    if (reverseGeocode) {
      try {
        ...
        searchError.value =
          "Could not get address details. Please enter city and zip code manually.";
      }
    }
    ...
    if (results && results.length > 0) {
      ...
      if (result.address) {
        selectedLocation.value = {
          ...
          zipCode:
            result.address.postcode || selectedLocation.value?.zipCode || "",
          latitude: lat,
          longitude: lng,
        };
        updateLocation();
      ...
      searchError.value =
        "No results found for this search. Please try a different location.";
    }
  } catch (error) {
